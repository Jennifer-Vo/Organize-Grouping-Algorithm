...
def windows(lst: List[Any], n: int) -> List[List[Any]]:
    ...
    if n >= len(lst):
        return [lst]
    res = list()
    for start in range(0, len(lst) - n + 1):
        res.append(lst[start: start + n])
    return res
...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        grouping = Grouping()
        students_groups_lst = slice_list(sorted_students, self.group_size)
        for lst in students_groups_lst:
...
class RandomGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        grouping = Grouping()
        students_groups_lst = slice_list(students, self.group_size)
        for lst in students_groups_lst:
            grouping.add_group(Group(lst))
        return grouping
...
class Group:
    ...
    def __str__(self) -> str:
        ...
        """
        s = ''
        for i, student in enumerate(self._members):
            if i != 0:
                s += ' '
            s += student.name
        return s
